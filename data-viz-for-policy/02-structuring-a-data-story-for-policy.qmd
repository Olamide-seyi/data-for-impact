...
# So What chart — dumbbell using pure ggplot2
chart_sowhat <- ggplot(zone_data) +
  # The connecting line
  geom_segment(
    aes(
      x     = penta3,
      xend  = penta1,
      y     = reorder(zone, penta1),
      yend  = reorder(zone, penta1)
    ),
    colour    = "#DDDDDD",
    linewidth = 2
  ) +
  # Penta3 dot — where children end up
  geom_point(
    aes(x = penta3, y = reorder(zone, penta1)),
    colour = "#D85A30",
    size   = 5
  ) +
  # Penta1 dot — where children start
  geom_point(
    aes(x = penta1, y = reorder(zone, penta1)),
    colour = "#534AB7",
    size   = 5
  ) +
  # Gap labels
  geom_text(
    aes(
      x     = penta1 + 2,
      y     = reorder(zone, penta1),
      label = paste0(round(dropout, 1), "pp gap")
    ),
    size   = 3,
    colour = "#888888",
    hjust  = 0
